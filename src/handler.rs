use crate::{
    model2::{NoteModel, NoteModelResponse},
    schema2::{CreateNoteSchema, FilterOptions, UpdateNoteSchema},
    AppState,
};
...
pub async fn note_list_handler( 
    ...
    let notes: Vec<NoteModel> = sqlx::query_as(
        r#"SELECT * FROM notes ORDER by id LIMIT ? OFFSET ?"#,).bind(limit as i32).bind(offset as i32)
    .fetch_all(&data.db)
    .await
    .unwrap();
...
async fn create_note_handler(
    ...
    let query_result = sqlx::query_as::<_, NoteModel>(r#"SELECT * FROM notes WHERE id = ?"#).bind(user_id)
        .fetch_one(&data.db)
        .await;
...
async fn get_note_handler(
    ...
    let query_result = sqlx::query_as::<_, NoteModel>(r#"SELECT * FROM notes WHERE id = ?"#).bind(note_id.clone())
        .fetch_one(&data.db)
        .await;
    ...
    match query_result {
        ...
        Err(sqlx::Error::RowNotFound) => {
            ...
            serde_json::json!({"status": "fail","message": format!("Note with ID: {} not found", note_id.clone())}),
        );
        }
        Err(e) => {
            return HttpResponse::InternalServerError()
...
async fn edit_note_handler(
    ...
    let updated_note_result: Result<NoteModel, sqlx::Error> = sqlx::query_as::<_, NoteModel>(
        r#"SELECT * FROM notes WHERE id = ?"#).bind(note_id.to_owned())
    .fetch_one(&data.db)
    .await;
...
async fn delete_note_handler(
    ...
    let note_id = path.into_inner().to_string();
    let query_result = sqlx::query(r#"DELETE FROM notes WHERE id = ?"#).bind(note_id.clone())
        .execute(&data.db)
        .await;
    ...
    match query_result {
        Ok(result) => {
            if result.rows_affected() == 0 {
                let message = format!("Note with ID: {} not found", note_id.clone());
