...
impl GroupDatabase {
    pub async fn create_group(group: web::Json<Group>, data:web::Data<AppState>) -> Result<Group, Error> {  //impl Responder {
...
//                return HttpResponse::BadRequest().json(
  //              serde_json::json!({"status": "fail","message": "Group with that title already exists"}),
        ...
        let query_result = sqlx::query(Group, r#"SELECT * FROM group WHERE id = ?"#, id)
        .fetch_one(&data.db)
        .await;
