...
use crate::models::usher_group::{UsherGroupModel, UsherGroupModelResponse, CreateUsherGroupSchema, ResponseCall};
...
//        let query_result = sqlx::query(r#"SELECT * FROM group WHERE id = ?"#).bind(id)
        ...
        //    Ok(group);
            ...
            let group_response: Response = serde_json::json!({"status": "success","data": serde_json::json!({
       ...
       //  let group_response = r#"{"status": "success","data": serde_json::json!({
         ...
         //     let test = r#"{"hello":"sdfaf"}"#;
            ...
            let group_json = serde_json::from_value(group_response);
